...
interface LogConfig {
    ...
    /**
     * @description: 最大文件大小，单位字节。默认为 512 * 1024 (512KB)。
     */
    maxFileSize?: number;
...
declare interface Console {
    ...
    /**
     * @description: 设置控制台的大小，单位像素。建议在显示控制台后等待一段时间后在进行调整，否则可能会设置失败。
     * @param {number} w 宽度
     * @param {number} h 高度
     * @example: 
     * ```typescript
    ...
    /**
     * @description: 设置控制台左上角的位置，单位像素。建议在显示控制台后等待一段时间后在进行调整，否则可能会设置失败。
     * @param {number} x 横坐标
     * @param {number} y 纵坐标
     * @example: 
     * ```typescript
    ...
    /**
     * @description: 设置日志保存的路径和配置。**注意: 该函数会影响所有脚本的日志记录。**
