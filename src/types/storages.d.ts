...
declare module 'storages' {
    ...
    class Storage {
         ...
         * - **注意！：返回的数据可能是任意数据类型，这取决于使用 `Storage.put` 保存该键值的数据时的数据类型。**
         ...
         * - **注意！：存储的过程实际上是使用 `JSON.stringify` 把 `value` 转换为字符串再保存，因此 `value` 必须是可 JSON 化的才能被接受。
         * @param {string} key 键名
