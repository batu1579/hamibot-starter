...
declare module "widget-operation" {
    global {
        ...
        namespace auto {
            ...
            function service(): true | null;

            function windows(): object[];

            function root(): UiObject | null;

            function rootInActiveWindow(): UiObject | null;

            function windowRoots(): UiObject | null;
             ...
             * - **注意！： setWindowFilter 函数会影响auto.windowRoots的结果。**
        ...
        interface SimpleActionAutomator {
             ...
             * - **注意！： 这里的输入文本的意思是，把输入框的文本置为 `text` ，而不是在原来的文本上追加。**
        ...
        class UiSelector {
             ...
             * @param {string} algorithm 搜索算法，可选值：
             * - `DFS` - 深度优先算法（默认）
             * - `BFS` - 广度优先算法
             ...
             * - **注意！：参数的这四个数字不能随意填写，必须精确的填写控件的四个边界才能找到该控件。**
            ...
            /**
             * @description: 根据当前的选择器所确定的筛选条件，对屏幕上的控件进行搜索，直到找到至少一个满足条件的控件为止，并返回所有满足条件的控件集合。该函数与find()函数的区别在于，该函数永远不会返回空集合；
             * -**注意！：如果屏幕上一直没有出现满足条件的控件，该函数会保持阻塞。**
             * @return {UiCollection} 根据选择器查找到的控件集合
             */
        ...
        class UiObject {
             ...
             * - **注意！：该函数只对可编辑的输入框( `editable` 为 `true` )有效。**
             ...
             * - **注意！：如果一个控件既可以向右滑动和向下滑动，那么执行 `scrollForward()` 的行为是未知的。因为 Android 文档没有指出这一点，同时也没有充分的测试可供参考。**
             * @return {boolean} 操作是否成功
             */
             ...
             * - **注意！：该函数在安卓 7.0 及以上才有效，7.0 以下版本调用会返回 0。**
        ...
        class UiCollection extends Array<UiObject> {
            ...
            /**
             * @description: 获取集合中的控件( `UiObject` )。历史遗留函数，建议直接使用数组下标的方式访问元素。。
            ...
            /**
             * @description: 点击集合内的所有控件，并返回是否点击成功。如果该函数返回 `false` ，可能是该控件不可点击( `clickable` 为 `false` )，当前界面无法响应该点击等。
             * @return {boolean} 操作是否成功
             */
            click(): boolean;

            /**
             * @description: 长按集合内的所有控件，并返回是否点击成功。如果该函数返回 `false` ，可能是该控件不可点击( `longClickable` 为 `false` )，当前界面无法响应该点击等。
             * @return {boolean} 操作是否成功
             */
            longClick(): boolean;
            ...
            toArray(): any;
            contextClick(): any;
            clearAccessibilityFocus(): any;
            accessibilityFocus(): any;
            dismiss(): any;
            scrollTo(): any;
            contains(): any;
            wait(): any;
