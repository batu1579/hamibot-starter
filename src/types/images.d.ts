...
declare module 'images' {
    ...
    global {
         ...
         * 
         * - **注意！：image 对象创建后尽量在不使用时进行回收，同时避免循环创建大量图片。Image 对象通过调用 `recycle()` 函数来回收。**
         * - **注意！： `caputerScreen()` 返回的图片不需要回收。**
         * @example
         * ```typescript
         * // 读取图片
         ...
         * - *建议（：第一次使用该函数会弹出截图权限请求，在对话框中选择总是允许。*
         * - *建议（：在本软件界面运行该函数，在其他软件界面运行时容易出现一闪而过的黑屏现象。*
         * - **注意！：如果不指定 landscape 值，则截图方向由当前设备屏幕方向决定，因此务必注意执行该函数时的屏幕方向。**
         * - **注意！：这个函数只是申请截图权限，并不会真正执行截图，真正的截图函数是 `captureScreen()` 。**
    ...
    class BaseImage {
         ...
         * - **注意！：坐标系以图片左上角为原点。以图片左侧边为 y 轴，上侧边为 x 轴。**
         * @param {number} x 横坐标。
         * @param {number} y 纵坐标。
         * @return {number} 点 (x, y) 处的像素的 ARGB 值。该值的格式为 0xAARRGGBB，是一个'32 位整数'（虽然 JavaScript 中并不区分整数类型和其他数值类型）。
         */
    ...
    interface TemplateOptions {
         ...
         * - **注意！：一般而言不必修改此参数。不加此参数时该参数会根据图片大小自动调整。**
         */
        level?: number;
